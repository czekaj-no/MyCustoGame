...
urlpatterns = [
    ...
    path('zamowienie/', views.checkout, name='checkout'),
    path('sklep/konto/', views.account_redirect, name='account_redirect'),
    ...
    path('logowanie/', views.custom_login_view, name='login'),
    path('rejestracja/', views.register_view, name='register'),
    path('konto/wyloguj/', LogoutView.as_view(next_page='login'), name='logout'),
    ...
    path('konto/reset/', auth_views.PasswordResetView.as_view(template_name='store/password_reset.html'), name='password_reset'),
    path('konto/reset/wyslano/', auth_views.PasswordResetDoneView.as_view(template_name='store/password_reset_done.html'), name='password_reset_done'),
    path('konto/reset/<uidb64>/<token>/', auth_views.PasswordResetConfirmView.as_view(template_name='store/password_reset_confirm.html'), name='password_reset_confirm'),
    path('konto/reset/zakonczono/', auth_views.PasswordResetCompleteView.as_view(template_name='store/password_reset_complete.html'), name='password_reset_complete'),
